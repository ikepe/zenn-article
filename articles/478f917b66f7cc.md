...
> **オプション設定**: `"latex-workshop.latex.autoBuild.run": "onFileChange"` を `"onSave"` に変更すると、ファイル保存時のみビルドが実行されます。編集中の頻繁なビルドを避けたい場合に推奨します。
...
**問題**: macOSでビルドが失敗する（`ptex2pdf: command not found`）
**解決法**:
LaTeX Workshopの設定に以下の`env`設定を追加してPATHを通します：
```json
{
    "latex-workshop.latex.tools": [
        {
